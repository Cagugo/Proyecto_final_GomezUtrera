...
router.get('/:pid', async (req, res) => {
  try {
    ...
    return res.status(500).json({ status: 'error', error: 'Error obtaining the product' });
  }
});
router.post('/', async (req, res) => {
  try {
    ...
    if (existingProduct) {
      return res.status(400).json({ status: 'error', error: 'A product with the same code already exists' });
    }
...
router.delete('/:pid', async (req, res) => {
  try {
    ...
    await fs.writeFile(productosFilePath, JSON.stringify(products, null, 2));
    return res.status(200).json({ status: 'success', message: 'Product disposed correctly' });
  } catch (error) {
